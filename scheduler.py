...
def event_date_for(person: dict, today: date) -> date:
    """Дата дня рождения в текущем году. Для 29.02 в невисокосный — 28.02."""
...
async def check_and_send(bot, now_utc: datetime | None = None) -> None:
    ...
    Запускается по расписанию (GitHub Actions cron раз в час). Обращения к БД —
    синхронные (libsql/sqlite3); отправка сообщений — async через aiogram.
    ...
    for u in db.all_users():
        ...
        for day, month in _birthday_candidates(today_local):
            people.extend(db.get_people_with_birthday_on(u["chat_id"], day, month))

        for person in people:
            event_date = event_date_for(person, today_local)
            event_key = event_date.isoformat()
            if db.already_sent(person["id"], event_key):
                continue

            wishes = db.list_wishes(person["id"])
            ...
            try:
                ...
                # чтобы не пытаться снова каждый час.
                logger.warning("send_message forbidden for chat_id=%s", u["chat_id"])
                _mark_sent_safe(person["id"], event_key)
...
def _mark_sent_safe(person_id: int, event_key: str) -> None:
    """Помечает отправленным; гонка (дубль) тихо игнорируется."""
    try:
        db.mark_sent(person_id, event_key)
